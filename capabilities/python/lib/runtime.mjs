...
// hermetic interpreter under ~/.wfc/bin/python using `uv`:
//
//   ~/.wfc/bin/python/
...
// present, otherwise download a static build straight into ~/.wfc/bin —
...
//
// The fix stays inside ~/.wfc, with no admin and no system-wide installer:
...
export function pythonRuntime(workspaceRoot = '') {
  ...
  async function provisionVenv(name, packages, python) {
    ...
    const marker = path.join(venvDir(name), '.wfc-install.json')
  ...
  async function ensureWindowsNativeRuntime(name) {
    ...
    try {
      ...
      const marker = path.join(venvDir(name), '.wfc-vcrt.json')
