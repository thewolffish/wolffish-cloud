...
tools:
  - name: python_check
    ...
    parameters: {}
  - name: python_install
    description: Install the managed Python runtime (fetch uv if needed, then a pinned no-root CPython under ~/.wfc/bin/python).
    parameters: {}
confirm_patterns:
...
system Python: everything lives under `~/.wfc/bin/python`, managed by
...
Each consumer gets its own venv under `~/.wfc/bin/python/venvs/<name>`, so
