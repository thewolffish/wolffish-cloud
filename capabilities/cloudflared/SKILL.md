...
tools:
  ...
  - name: cloudflared_install
    description: Install cloudflared with no admin rights — reuse a global cloudflared, else download the official no-root binary into ~/.wfc/bin
    parameters: {}
...
the official no-root binary into `~/.wfc/bin/cloudflared`. Only use
