...
// No-root managed location, sibling of node/ffmpeg/python under ~/.wfc/bin.
const CLOUDFLARED_DIR = path.join(homedir(), '.wfc', 'bin', 'cloudflared')
...
const toolDefinitions = [
  ...
  },
  {
    ...
    description:
      'Install cloudflared with no admin rights: reuse a global cloudflared, else download the official no-root binary into ~/.wfc/bin. No package manager, no password.',
...
function describeAction(toolName, args) {
  ...
  if (toolName === 'cloudflared_install') {
    return {
      ...
      command: 'download cloudflared → ~/.wfc/bin/cloudflared',
