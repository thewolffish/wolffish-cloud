...
// packages such as Node live under ~/.wfc/bin (a sibling of the workspace,
// alongside any other no-root tools), so the tree sits at ~/.wfc/bin/node.
// ---------------------------------------------------------------------------
...
async function nodeInstall() {
  ...
  // node (>= floor), else download a no-root managed copy into ~/.wfc/bin.
  ...
  if (usable) {
    return {
      ...
      output: usable.managed
        ? `Using managed Node.js ${usable.version} (no-root, in ~/.wfc/bin)`
...
const toolDefinitions = [
  ...
  },
  {
    name: 'node_install',
    description:
      'Provision Node.js with no admin rights: reuse an existing Node 24+ if present, otherwise download an official no-root copy into ~/.wfc/bin. No package manager, no password prompt.',
    parameters: { type: 'object', properties: {}, required: [] }
  },
  {
...
function describeAction(toolName) {
  ...
  if (toolName === 'node_install') {
    return {
      ...
      command: 'download node-vXX-<os>-<arch> → ~/.wfc/bin/node',
      impact:
