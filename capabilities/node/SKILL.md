...
tools:
  - name: node_check
    ...
    parameters: {}
  - name: node_install
    description: Provision Node.js with no admin rights — reuse Node 24+ if present, else download an official no-root copy into ~/.wfc/bin
    parameters: {}
...
Node into `~/.wfc/bin`. No package manager, no password prompt. This is the
