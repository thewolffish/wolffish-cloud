...
const FFMPEG_DIR = path.join(homedir(), '.wfc', 'bin', 'ffmpeg')
...
const toolDefinitions = [
  ...
  },
  {
    name: 'ffmpeg_install',
    description:
      'Install ffmpeg with no admin rights: reuse a global ffmpeg, else download a no-root static build into ~/.wfc/bin. No package manager, no password.',
...
function describeAction(toolName, args) {
  ...
  if (toolName === 'ffmpeg_install') {
    return {
      ...
      command: 'download ffmpeg → ~/.wfc/bin/ffmpeg',
      impact:
