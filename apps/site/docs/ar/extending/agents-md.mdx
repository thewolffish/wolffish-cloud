---
title: "AGENTS.md"
description: "دليل جاهز للّصق يعلّم أي مساعد ذكاء اصطناعي كيف يتنقّل في مجلد ~/.wolffish الخاص بك"
---

# ملف واحد لتوجيه أي مساعد

`AGENTS.md` هو ملف Markdown واحد ومكتفٍ بذاته تضعه في جذر مجلد بياناتك — `~/.wolffish/AGENTS.md` — **ليوجّه أي مساعد ذكاء اصطناعي توجّهه إلى تثبيت وولفيش لديك**. افتح `.wolffish` في محرّر، أو سلّم المجلد لوكيل برمجي (Claude Code أو Cursor أو مساعد متصل عبر MCP)، فيقرأ هذا الملف الواحد ليفهم فورًا ما هو وولفيش، وأين يوجد كل شيء، وكيف يساعدك.

<Note>
  لا تخلط بينه وبين [`agents.md`](/ar/configuration/agents-md) (بحروف صغيرة، في `brain/prefrontal/agents.md`). ذلك الملف يخبر **وكيل وولفيش الخاص بك** كيف يؤدّي عمله. أما **`AGENTS.md`** (بحروف كبيرة، في جذر المجلد) فيخبر مساعدًا **زائرًا** كيف يتنقّل في *مساحة عملك*. جمهور مختلف، وملف مختلف.
</Note>

## لماذا تضعه

مساحة عمل وولفيش ما هي إلا ملفات — قابلة للقراءة والتحرير، وملكك. هذا هو بيت القصيد: يمكنك إحضار أي ذكاء اصطناعي ليساعدك على قراءتها أو إصلاحها أو توسيعها. لكن مساعدًا باردًا لا يعرف أن `conversations/` تقع تحت `brain/`، ولا أن القدرات مسبوقة بنقطة، ولا أن `cortex.db` قابل للحذف. يضع `AGENTS.md` كل ذلك في المقدّمة ليصبح المساعد مفيدًا من الرسالة الأولى لا العاشرة.

لحظات يثبت فيها جدواه:

- *"لماذا فعل وكيلي ذلك؟"* — يعرف المساعد أن يقرأ `brain/corpus/` و`brain/prefrontal/.debug/`.
- *"غيّر شخصية وكيلي."* — يحرّر `brain/identity/soul.md`، لا كودًا عشوائيًا.
- *"كم أُنفق؟"* — يجمع `usage/daily/*.md`.
- *"أضف أداة تتحقق من الطقس."* — يبني قدرة بالطريقة الصحيحة (انظر أدناه).

## ما بداخله — المواضيع التي يغطّيها

الملف منظّم في اثني عشر جزءًا. إليك ما يعلّمه كل جزء للقارئ:

| # | القسم | ما يعلّمه |
|---|---|---|
| 1 | **الفكرة** | الفلسفة — محلي أولًا، والـ Markdown هو الحقيقة، ودماغ تملكه |
| 2 | **حزمة التقنيات** | Electron / React / TS، وقائمة المزوّدين، والمكتبات الأساسية |
| 3 | **مجلد `.wolffish` — الخريطة الكاملة** | شجرة موثّقة لكل مجلد وما يحويه |
| 4 | **أين يوجد كل شيء** | جدول بحث "المستخدم يريد كذا ← حرّر هذا الملف" |
| 5 | **الدماغ — 15 وحدة** | مهمة كل وحدة وموقعها على القرص، إضافة إلى خط الأنابيب |
| 6 | **الأدوات المتاحة (القدرات)** | الفهرس الكامل للقدرات المدمجة وكيفية بنائها |
| 7 | **كيف تعمل الأمور** | خط الأنابيب، ونموذج الدماغ، والذاكرة ثلاثية الطبقات، والأمان، ونبضة القلب، والقنوات |
| 8 | **مرجع تنسيقات الملفات** | شكل `config.json` والمحادثات والحلقات والمهام والسجلات والاستخدام |
| 9 | **وصفات التنقّل** | خطوات عملية للأمور التي يسألها المستخدمون فعلًا |
| 10 | **كتابة موجّهات جيدة** | كيف توجّه وكيلًا يملك ذاكرة وأدوات (بما في ذلك التشغيل الذاتي) |
| 11 | **الأمان والتحكّم** | ما الذي تحرسه اللوزة، وكيف يغيّره `bypassPermissions` |
| 12 | **القواعد الذهبية** | القواعد الصارمة: الـ Markdown هو الحقيقة، ابقَ داخل المجلد، احمِ الأسرار |

اقرأه من أعلى إلى أسفل فيكون دورة تعريفية. واستخدمه كمرجع فيجيب القسمان **4** (أين يوجد كل شيء) و**8** (تنسيقات الملفات) عن معظم أسئلة "أين / ماذا" وحدهما.

## إنشاء قدرة — النسخة المختصرة

من أكثر ما ستطلبه من مساعد زائر: *"علّم وولفيش مهارة جديدة."* الشرح الكامل — مع مثال طقس **عامل** لا يحتاج إلى مفتاح API — موجود في **[إنشاء القدرات](/ar/capabilities/creating-capabilities)**. لكن إليك الفكرة كلها في أربع خطوات، وهي تقريبًا ما يخبر به `AGENTS.md` المساعد:

<Steps>
  <Step title="أنشئ مجلدًا">
    `~/.wolffish/workspace/brain/cerebellum/<name>/`
  </Step>
  <Step title="اكتب SKILL.md">
    واجهة YAML (`name` و`description` و`triggers` و`tools`) إضافة إلى متن Markdown من التعليمات. أضف `danger_patterns` / `confirm_patterns` **فقط** إذا كانت الأداة قادرة على إحداث ضرر.
  </Step>
  <Step title="أضف plugin/index.mjs (اختياري)">
    للمنطق المخصص، صدّر `{ name, tools, execute(toolName, args) }` تُرجع `{ success, output }`. المهارات النقية التي تعتمد على أدوات موجودة (`shell_exec`، `file_read`) تتخطّى هذا تمامًا.
  </Step>
  <Step title="أعد تشغيل وولفيش">
    يكتشف المخيخ المجلد، وتجعله `triggers` قابلًا للمطابقة، وتصبح `tools` قابلة للاستدعاء. دون إعادة بناء التطبيق.
  </Step>
</Steps>

هذا هو نموذج التوسيع كله: **مجلد من Markdown يعلّم وكيلك حيلة جديدة.** انظر [كتابة الإضافات](/ar/capabilities/writing-plugins) و[أنماط الأمان](/ar/extending/safety-patterns) للتعمّق.

## أين يوجد

يضمّن وولفيش هذا الدليل ويكتبه في **جذر** مجلد بياناتك، ويحدّثه عند كل إقلاع:

```bash
~/.wolffish/AGENTS.md
```

يقع بجوار `workspace/` و`runtime/` و`logs/` — *وليس* داخل `workspace/`. هذا يبقيه بعيدًا عن إعادة تهيئة مساحة العمل، ويجعله أول ما يراه أي شخص يفتح `~/.wolffish/`.

<Tip>
  هذا الملف **يديره وولفيش** — تُعاد كتابة النسخة المضمّنة عند كل إقلاع، فتحدّثه ترقية التطبيق دائمًا إلى الإصدار الحالي. عامله كملف للقراءة فقط: فأي تعديل محلي يُستبدل عند الإقلاع التالي. وللسلوك المخصص للوكيل الذي يجب أن *يبقى*، استخدم [`agents.md`](/ar/configuration/agents-md) بدلًا منه.
</Tip>

## ملف AGENTS.md الكامل

<Note>
  ملف `AGENTS.md` نفسه مكتوب بالإنجليزية عمدًا: فهو أداة تقنية يقرؤها مساعد ذكاء اصطناعي، ومصطلحاته ومسارات ملفاته إنجليزية. انسخه كما هو.
</Note>

هذا هو النص الذي يضمّنه وولفيش ويبقيه متزامنًا في `~/.wolffish/AGENTS.md`، معروض هنا للرجوع إليه — اقرأه، أو ضعه يدويًا في تثبيت أقدم.
...
│                          (messages, segments, timeline, stats — see §8).
│
├── motor/                 EXECUTION.
...
| **Cortex** | Fast full-text retrieval over everything the agent has done | `brain/cortex.db` |
...
| **Meta** | `.introspect` | The agent recalls & inspects itself — `memory_search`, `memory_get`, `conversation_list`, `conversation_read`, `memory_save`, `usage_report`, `wolffish_recall`, `wolffish_status`, `channel_status`, `wolffish_performance`, `wolffish_memory`, `wolffish_list_files` |
| | `.skills` | The agent manages/authors its own capabilities — `skill_list`, `skill_search`, `skill_read_source`, `skill_enable`, `skill_disable`, `skill_delete`, `skill_create`, `skill_reload` |
...
triggers:            # search terms tool_search matches to discover this skill
  - run
  - execute
  - terminal
...
`plugin/index.mjs`). On next launch the cerebellum loads it; it appears in the
agent's capability index, `tool_search` finds it (matching its name, description,
and `triggers`), and its `tools` become callable. No app rebuild needed — that's
the markdown-as-truth payoff.
...
the pipeline in §5. The prefrontal assembles one lean system prompt — `soul.md`,
`user.md`, the agent procedures, a one-line-per-capability index, and a map of
what memory exists on disk. Nothing is dumped in wholesale: the agent retrieves
memory (`memory_search`) and loads tools (`tool_search`) on demand as the turn
needs them. The model streams a response; tool calls are parsed out,
safety-checked, executed with retries, and the result is streamed back. The turn
is saved to an episode and the outcome recorded.

**The Brain model.** You explicitly select one model — the **Brain**
(`config.llm.brain`, a `{ providerId, model }` pair) — from the chat composer's
model switch, and that is the model the thalamus runs. There's no cascade and no
automatic substitution between providers: on a transient error the thalamus
retries the **same** Brain on a backoff schedule (using `net.isOnline()` for
instant offline detection) rather than falling through to a different one. To
run offline, flip the model switch to Local (`llm.localOnly`). Workflow mode
(`llm.mode: "workflow"`, the **Mode** chip row in the model card) lets each turn plan phases
and spawn parallel agents, the master picking every agent's model from the
connected providers at spawn time; every model that runs is an explicit choice,
still no cascade.
...
- *Consolidated* (`hippocampus/consolidated/`) — weekly LLM summaries, written by
  nightly compaction (configured in Settings → Knowledge → Compaction, not in
  `heartbeat.md`).
- *Knowledge* (`hippocampus/knowledge/`) — durable facts, promoted from episodes or
  written by hand. All three tiers are indexed by the cortex; none of it is injected
  into the prompt. The prompt carries a coverage *map* of what exists, and the agent
  retrieves what a turn needs with `memory_search` / `memory_get`.
...
`TurnRunner` serializes turns **per conversation** (same conversation queues,
different conversations run in parallel) so a transcript never interleaves while
unrelated chats proceed concurrently; a `TurnRouter` sends approval prompts back
to whichever channel owns each turn.

**Conversations & the context meter.** Each conversation JSON carries a
`stats` block (lifetime totals, the last turn's roll-up, and a `meter` of
`contextTokens` vs `contextBudget`, e.g. 108 867 / 1 000 000) and a
...
```jsonc
{
  ...
  "llm": {
    ...
    "mode": "single",                                                    // chat mode: "single" or "workflow"
...
**For everyday asks, just talk.** Natural language is enough — the agent discovers
the right capability itself, via its capability index and `tool_search` ("summarize
this PDF", "commit my changes", "what's on my calendar tomorrow"). Don't name
tools; describe the outcome.
