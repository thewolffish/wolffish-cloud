...
Wolffish Mobile lives on the stores — [**the App Store**](https://apps.apple.com/us/app/wolffish/id6792797989) for iPhone and iPad, [**Google Play**](https://play.google.com/store/apps/details?id=sh.wolffi.mobile) for Android. The desktop's **Settings → Channels → Mobile** panel carries a **"Get the app" card** holding the same two links, so the path reads the way it runs: install, open, scan — with nothing to hunt for in between.
...
| **Read and change settings** | Every row the phone shows takes an edit — chat mode and thinking, the model and provider (a fresh key included), MCP switches, the compaction and [reflection](/memory/reflection) schedules, channel preferences and allow-lists, voice replies, transcription language, week start — each landing in **the desktop's own setters** |
...
| **Drive the desktop's updater** | Check and install are taps, mirrored live — see [Updates](#updates) |
...
Because a turn started on the phone is a real turn, it gets its own **channel badge**: conversations that began there wear a phone glyph in the desktop's History list and conversations sheet, next to the Telegram, WhatsApp, [terminal](/getting-started/cli), automation and procedure badges.
...
Work running on the desktop can show on the phone as a **card in a stack over whatever screen you are on**, with the prompt behind each one a tap away: an [automation](/configuration/heartbeat) or [procedure](/configuration/procedures) mid-run, a [compaction](/memory/consolidation), the [nightly reflection](/memory/reflection). Since **v1.0.255** every family of run carries **its own switch, and they all start off**: automations and procedures share one on **Settings → Channels**, compaction and reflection each have their own on **Settings → Knowledge**. Off hides the card and nothing else — the run happens on the same schedule, notifications still arrive, and the Automations and Knowledge screens still report what ran. The phone's switches are **separate from the desktop's own**, because a card worth having on your desk is not automatically one worth having in your pocket. The one card that never hides is the **memory index rebuild** — that one really does block the desktop, and a phone left guessing why nothing answers is worse than a card. Cards appear the moment the phone connects — including for a run that started while it was asleep — and they go when the connection goes, because a card claiming something is running on a machine the phone can no longer see is a card that lies.
