...
export async function status(client, { json, raw = false } = {}) {
  ...
  // The account, before the machine facts: nothing below works signed out.
  ...
  keyValue([
    [
      'status',
      auth?.status === 'ready'
        ? c.green(describeAccount(auth))
        : auth?.status === 'locked'
          ? c.yellow(describeAccount(auth))
          : c.yellow(describeAccount(auth))
    ],
    ...(auth?.user?.email ? [['email', c.gray(auth.user.email)]] : []),
    ...(auth?.orgName ? [['organization', auth.orgName]] : [])
  ])
  const gate = authGate(auth)
  if (gate) out(c.gray(`    ${gate}`))
  ...
  if (!p.installed) {
    ...
    else out(`  ${icon.warn()} ${c.yellow('run: wfc path install')}`)
  }
...
export async function pathCommand(client, args) {
  ...
  if (sub === 'install') {
    ...
    if (state.needsPathEntry && state.profileHasEntry) {
      out(`${icon.ok()} added to your shell profile — open a new terminal and \`wfc\` is there`)
      return 0
    }
