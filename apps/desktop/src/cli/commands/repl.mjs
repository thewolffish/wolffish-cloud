...
const SLASH_HELP = [
  ...
  ['ACCOUNT', null],
  ['/login [email]', 'sign in — email + password, then your PIN'],
  ['/logout', 'sign out and clear this machine'],
  ['/unlock', 'the PIN door'],
  ['/lock', 'lock now'],
...
async function handleSlash(client, state, input) {
  ...
  switch (command) {
    ...
    case 'unlock': {
      const { unlock } = await import('./auth.mjs')
      await unlock(client)
      return null
    }
    case 'lock': {
      const { lock } = await import('./auth.mjs')
      await lock(client)
      return null
    }
    case 'account':
