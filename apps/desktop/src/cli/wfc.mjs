...
/**
 * `wfc status | head -3` is a normal thing to type, and it used to end in
 ...
 */
const USAGE = `
${c.bold('wfc')} — your agent, in the terminal

${c.gray('CHAT')}
  wfc                          open a session
  wfc "<prompt>"               ask once, print, exit
  wfc -p "<prompt>" -f a.pdf   attach files by path
  wfc -p "…" --project <id>   ask inside a project
  wfc -p "…" --plan           plan mode: read-only turn that only writes its plan file
  cat log.txt | wfc -p "why?"  pipe context in
  wfc resume [id]              continue a past conversation
...
${c.gray('THE APP, IN THE TERMINAL')}
  ...
  wfc automations              browse ${c.gray('·')} show <label> ${c.gray('·')} run <label> ${c.gray('·')} edit ${c.gray('·')} copy ${c.gray('·')} paste
  wfc customizations           soul · user · agents ${c.gray('(view; edit · copy · paste <name>)')}
  wfc settings                 page ${g.chevron} card ${g.chevron} setting, interactively

${c.gray('FILES AND FOLDERS — projects, procedures and automations alike')}
  wfc <thing> files <id>              what it carries
  wfc <thing> files <id> add <path…>  attach (copied into the workspace)
  wfc <thing> files <id> rm <name…>   detach
  wfc <thing> dirs <id> add <path…>   work in a folder
...
${c.gray('WORKSPACE')}
  wfc files                    walk it — folders open, files open
  wfc view <path>              read one
  wfc edit <path>              change one

${c.gray('ACCOUNT')}
  wfc login [email]            email + password, then your PIN ${c.gray('(temporary password → new one)')}
  wfc logout                   revoke the session, clear this machine
  wfc unlock | lock            the PIN door ${c.gray('(five misses sign you out)')}
  wfc account                  who is signed in ${c.gray('· --json')}
  wfc reset-password [email]   a 6-digit code by email, then a new password
  wfc activate [email]         invited account: the code from the email, then a password
  wfc change-password | change-pin
...
/**
 * Prompt context piped in, if any — lets `cat log | wfc -p "why?"` work.
 *
...
async function main() {
  ...
  if (command === 'version' || command === '--version') {
    ...
    if (!client) {
      out('wfc (daemon not running)')
      return 0
    }
    const info = client.hello
    out(`wfc ${info?.version ?? ''}`.trim())
    client.close()
    return 0
  }
      ...
      wrapText(
        c.gray(
          attemptedStart
            ...
            : // NOT "e.g. wfc status": status is one of the three that
  ...
  // it (`wfc settings`, or /verbose in a session).
...
async function dispatch(client, command, args, flags) {
  switch (command) {
    case undefined: {
      ...
      // pipe on stdin is one implicitly: a script that pipes into wfc
    ...
    case 'settings': {
      if (args[0] === 'get') {
        if (!args[1]) return usageError('wfc settings get <id>')
    ...
    case 'view':
    case 'cat':
      if (!args[0]) return usageError('wfc view <workspace path>')
      ...
      // so `wfc view x --json | jq` got raw file text and a parse error.
    ...
    case 'edit':
      if (!args[0]) return usageError('wfc edit <workspace path>')
    ...
    case 'logout':
    case 'signout':
      return logout(client, { yes: flags.yes === true })
    case 'unlock':
    ...
    case 'whoami':
    case 'me':
      return account(client, { json: flags.json })
    case 'reset-password':
    case 'forgot':
      return resetPassword(client, args)
    case 'activate':
      return activate(client, args)
    case 'change-password':
      return changePassword(client)
    case 'change-pin':
      return changePin(client)
     ...
     *
     * `wfc status` lists everything running on the box, including
    ...
    case 'cancel': {
      ...
      if (!args[0] && !flags.all) {
        ...
        out()
        out(c.gray('  wfc cancel <id>   ·   wfc cancel --all'))
        return 0
      }
    ...
    default: {
      ...
      /**
       * Anything unrecognized is a prompt, so `wfc "why is the disk full"`
       * works without remembering -p.
       *
       * With ONE exception: a single bare word, close to a command that exists.
       * `wfc setings` is a typo, not a question, and sending it as a
      ...
      if (typo) {
        ...
        err(c.gray(`  did you mean: wfc ${typo}`))
        err(c.gray(`  to ask it as a question instead: wfc -p "${command}"`))
        return 2
      }
...
async function oneShot(client, text, flags) {
  ...
  if (!prompt) return usageError('wfc -p "<prompt>"')
  ...
  if (!flags.json) {
    out()
    out(c.gray(`  wfc resume ${result.conversationId}`))
  }
  return 0
}
