...
| A message too large for one record (>300 KB) | The whole message spills to a content-addressed blob; the record carries a readable prefix and a pointer | Restored in full — the body is fetched with its record |
...
| `config.json` | One row, last-write-wins by server stamp; an admin can fix a user's config from the console and the app adopts it within ~2 minutes. Paths the ORG claims (its config overlay) are applied on every read and forced on every write — those settings are the organization's, and the app shows them as such | Adopted from the org before anything else; a fresh install never overwrites it with defaults |
