    ...
    (function(s, o) {
      ...
      if (globalThis.browser && globalThis.browser.runtime && globalThis.browser.runtime.id)
        ...
        const o = "The message port closed before a response was received.", n = (a) => {
          ...
          const c = (h) => h && typeof h == "object" && typeof h.then == "function", m = (h, p) => (...E) => {
          ...
          }, d = (h) => h == 1 ? "argument" : "arguments", l = (h, p) => function(_, ...y) {
            if (y.length < p.minArgs)
              throw new Error(`Expected at least ${p.minArgs} ${d(p.minArgs)} for ${h}(), got ${y.length}`);
            if (y.length > p.maxArgs)
              throw new Error(`Expected at most ${p.maxArgs} ${d(p.maxArgs)} for ${h}(), got ${y.length}`);
            return new Promise((C, O) => {
              if (p.fallbackToNoCallback)
                try {
                  ...
                  console.warn(`${h} API method doesn't seem to support the callback parameter, falling back to call it without a callback: `, w), _[h](...y), p.fallbackToNoCallback = !1, p.noCallback = !0, C();
                }
              else p.noCallback ? (_[h](...y), C()) : _[h](...y, m({
                resolve: C,
                reject: O
              }, p));
            });
          ...
          const f = (h, p = {}, E = {}) => {
            let _ = /* @__PURE__ */ Object.create(null), y = {
              has(O, w) {
                return w in h || w in _;
              },
              get(O, w, T) {
                if (w in _)
                  return _[w];
                ...
                if (typeof b == "function")
                  if (typeof p[w] == "function")
                    ...
                    b = b.bind(h);
                else if (typeof b == "object" && b !== null && (g(p, w) || g(E, w)))
                  b = f(b, p[w], E[w]);
                else if (g(E, "*"))
                  b = f(b, p[w], E["*"]);
                else
              ...
              set(O, w, T, b) {
                return w in _ ? _[w] = T : h[w] = T, !0;
              },
              defineProperty(O, w, T) {
                return Reflect.defineProperty(_, w, T);
              },
              deleteProperty(O, w) {
                return Reflect.deleteProperty(_, w);
              }
            }, C = Object.create(h);
            return new Proxy(C, y);
          }, S = (h) => ({
            addListener(p, E, ..._) {
              p.addListener(h.get(E), ..._);
            },
            hasListener(p, E) {
            ...
            h(_);
          }), R = new i((h) => typeof h != "function" ? h : function(E, _, y) {
            ...
            if (_.length < p.minArgs)
              throw new Error(`Expected at least ${p.minArgs} ${d(p.minArgs)} for ${h}(), got ${_.length}`);
            if (_.length > p.maxArgs)
              throw new Error(`Expected at most ${p.maxArgs} ${d(p.maxArgs)} for ${h}(), got ${_.length}`);
            return new Promise((y, C) => {
              const O = I.bind(null, {
                resolve: y,
                reject: C
              });
              _.push(O), E.sendMessage(..._);
            });
          }, N = {
            devtools: {
              network: {
                onRequestFinished: S(A)
              }
            },
            runtime: {
              onMessage: S(R),
              onMessageExternal: S(R),
...
const DEFAULT_PORT = 23152, LOG_PREFIX = "[Wolffish Cloud]", RECONNECT_ALARM_MINUTES = 0.5, HEARTBEAT_INTERVAL_MS = 15e3, COMMAND_TIMEOUT_MS = 3e4, CONTENT_SCRIPT_PING_TIMEOUT_MS = 500, RING_BUFFER_SIZE = 500, NETWORK_BODY_MAX_CHARS = 2e5, ACTION_NAV_START_MS = 200, ACTION_DOM_QUIET_MS = 120, ACTION_DOM_QUIET_MAX_MS = 1500, ACTION_NAV_SETTLE_MAX_MS = 5e3, OVERLAY_IDLE_MS = 2e4, OVERLAY_IDLE_ALARM_MINUTES = 0.5, OVERLAY_CURSOR_MIN_INTERVAL_MS = 16, STORAGE_KEY_OVERLAY_ENABLED = "wf:overlay-enabled", STORAGE_KEY_CDP_TABS = "wf:cdp-tabs", STORAGE_KEY_INUSE = "wf:inuse", STORAGE_KEY_SNAPSHOT_PREFIX = "wf:snap:", BRIDGE_TOKEN_FILE = "bridge-token.json", WolffishCommands = {
  ...
  await pingContentScript(e) || (await ((s = api$9 == null ? void 0 : api$9.scripting) == null ? void 0 : s.executeScript({
    ...
    var i, c;
    const a = setTimeout(() => n(new Error("Content script injection timed out")), 5e3), r = (m) => {
      var d, l;
      (m == null ? void 0 : m.source) === "content-script" && "type" in m.payload && m.payload.type === "pong" && (clearTimeout(a), (l = (d = api$9 == null ? void 0 : api$9.runtime) == null ? void 0 : d.onMessage) == null || l.removeListener(r), o());
    };
...
const checkStoragePermission = (e) => {
  ...
  var S, A;
  let o = null, n = !1, a = [];
  const r = (s == null ? void 0 : s.storageEnum) ?? StorageEnum.Local, i = ((S = s == null ? void 0 : s.serialization) == null ? void 0 : S.serialize) ?? ((R) => R), c = ((A = s == null ? void 0 : s.serialization) == null ? void 0 : A.deserialize) ?? ((R) => R);
  ...
  const m = async () => {
    ...
    const R = await (chrome$1 == null ? void 0 : chrome$1.storage[r].get([e]));
    return R ? c(R[e]) ?? t : t;
  }, d = async (R) => {
    n || (o = await m()), o = await updateCache(R, o), await (chrome$1 == null ? void 0 : chrome$1.storage[r].set({ [e]: i(o) })), g();
  }, l = (R) => (a = [...a, R], () => {
    a = a.filter((I) => I !== R);
  }), u = () => o, g = () => {
    a.forEach((R) => R());
...
`)[0].replace(/^(Error|TypeError|RangeError|SyntaxError|ReferenceError|DOMException): /, "");
  ...
  if (e.startsWith("text=")) {
    ...
    return c.filter((d) => !c.some((l) => l !== d && d.contains(l))).find(s) ?? null;
  }
  try {
  ...
  const a = e + (s - e) * 0.25 + (Math.random() - 0.5) * Math.abs(s - e) * 0.3, r = t + (o - t) * 0.25 + (Math.random() - 0.5) * Math.abs(o - t) * 0.3, i = e + (s - e) * 0.75 + (Math.random() - 0.5) * Math.abs(s - e) * 0.3, c = t + (o - t) * 0.75 + (Math.random() - 0.5) * Math.abs(o - t) * 0.3, m = [];
  for (let d = 1; d <= n; d++) {
    const l = d / n, u = 1 - l, g = u * u * u * e + 3 * u * u * l * a + 3 * u * l * l * i + l * l * l * s, f = u * u * u * t + 3 * u * u * l * r + 3 * u * l * l * c + l * l * l * o;
    m.push({ x: Math.round(g), y: Math.round(f) });
  }
  return m;
...
` || a === "\r" ? await pressEnter(o) : isPrintableAscii(a) ? await typeAsciiChar(o, a) : await sendCDP$1(o, "Input.insertText", { text: a }), await sleep$1(gaussianDelay(40, 120, 70));
  ...
  try {
    for (const f of n) {
      const S = MODIFIER_KEYS[f];
      a |= MODIFIER_FLAGS[f], await keyEvent(o, {
        type: "keyDown",
        key: S.key,
        code: S.code,
        windowsVirtualKeyCode: S.keyCode,
        nativeVirtualKeyCode: S.keyCode,
        modifiers: a
      }), r.push(f);
    }
    const i = SPECIAL_KEYS[t], c = t.length === 1, m = (i == null ? void 0 : i.code) ?? (c ? codeForChar(t) : t), d = (i == null ? void 0 : i.keyCode) ?? (c ? t.toUpperCase().charCodeAt(0) : 0), l = c && (a & (MODIFIER_FLAGS.meta | MODIFIER_FLAGS.ctrl)) !== 0, u = l ? EDIT_COMMANDS[t.toLowerCase()] : void 0, g = { key: t, code: m, windowsVirtualKeyCode: d, nativeVirtualKeyCode: d, modifiers: a };
    await keyEvent(o, {
      type: "keyDown",
      ...g,
  ...
  for (; o.length > 0; ) {
    ...
    for (let d = 0; d + 1 < i.length; d += 2) r[i[d]] = i[d + 1];
    ...
    for (const d of m) o.push({ node: d, inOverlay: c });
  }
  ...
  for (const n of o) {
    ...
    const c = `${r}:`, m = [];
    for (const d of i) {
      const l = c + d.nodeId, u = {
        ...d,
        nodeId: l,
        parentId: d.parentId ? c + d.parentId : n.nodeId,
        childIds: (d.childIds ?? []).map((g) => c + g)
      };
      d.parentId || m.push(l), t.set(l, u);
    }
    n.childIds = m;
  }
  ...
  const c = (l = e.value) == null ? void 0 : l.value, m = c !== void 0 && c !== "" ? String(c) : (s == null ? void 0 : s.attrs.value) ?? "";
  m && n.push(`value="${escapeText(m)}"`);
  const d = (s == null ? void 0 : s.attrs.placeholder) ?? "";
  return d && n.push(`placeholder="${escapeText(d)}"`), t === "link" && (s != null && s.attrs.href) && n.push(`href="${escapeText(clip(s.attrs.href, 200))}"`), (t === "image" || t === "img") && (s != null && s.attrs.src) && n.push(`url="${escapeText(clip(s.attrs.src, 120))}"`), n.length > 0 ? ` ${n.join(" ")}` : "";
}, hostOf = (e) => {
  try {
  ...
  let m = e.verbose || isInteresting(a, r);
  if (m && !e.verbose && a === "statictext" && o.includes(r.trim().toLowerCase()) && (m = !1), !m) {
    for (const f of n) walk(e, f, s, o);
    return;
  }
  const d = r ? ` "${escapeText(r)}"` : "", l = emit(e, t, s, `${a}${d}${attributesFor(t, a, i)}`);
  ...
  const m = await nextSnapshotId(e), d = {
    session: e,
    verbose: t,
    snapshotId: m,
    byId: c,
  ...
  for (const u of l) walk(d, u, 0, []);
  return e.uidMap = d.nextUidMap, e.uidByNode = d.nextUidByNode, e.snapshotNodes = d.nodes, e.hasSnapshot = !0, {
    snapshot: d.lines.join(`
`),
    url: (a == null ? void 0 : a.url) ?? "",
    title: (a == null ? void 0 : a.title) ?? "",
    nodeCount: d.lines.length,
    source: "cdp",
    snapshotId: m
  };
  ...
  const n = (a = (await api$6.scripting.executeScript({
    target: { tabId: e },
    func: (r, i) => {
      const c = (g) => g.replace(/\s+/g, " ").trim().toLowerCase(), m = (g) => {
      ...
      if (r.startsWith("text=")) {
        ...
        if (g) {
          const f = /* @__PURE__ */ new Set(["SCRIPT", "STYLE", "NOSCRIPT", "TEMPLATE", "WOLFFISH-OVERLAY"]), S = [], A = [], R = document.body ? Array.from(document.body.getElementsByTagName("*")) : [];
          for (const N of R) {
            const v = N;
            if (f.has(v.tagName)) continue;
            const h = c(v.textContent ?? "");
            !h || h.length > g.length + 200 || (h === g ? S.push(v) : h.includes(g) && A.push(v));
          }
          const I = S.length > 0 ? S : A;
          d = I.filter((N) => !I.some((v) => v !== N && N.contains(v))).find(m) ?? null;
        }
      } else
        try {
      ...
      if (!d) return null;
      i && d.scrollIntoView({ behavior: "smooth", block: "center" });
      const l = d.getBoundingClientRect(), u = d.closest("a");
      return {
  ...
  let c, m = r.cssLayoutViewport.clientWidth, d = r.cssLayoutViewport.clientHeight;
  if (t.uid || t.selector) {
    ...
    c = {
      ...
      scale: 1
    }, m = c.width, d = c.height;
  } else a && (c = { x: 0, y: 0, width: r.cssContentSize.width, height: r.cssContentSize.height, scale: 1 }, m = c.width, d = c.height);
  ...
  return {
    ...
    width: Math.round(m * i),
    height: Math.round(d * i),
    cssWidth: Math.round(m),
    cssHeight: Math.round(d),
    dpr: i,
    mode: "cdp"
  };
  ...
  if (Array.isArray(s) && s.length > 0) {
    ...
    try {
      for (const m of s) {
        const d = lookupUid(o, m);
        i.push(await withUidErrors(m, () => resolveObjectId(n, d.backendNodeId)));
      }
      let c;
      try {
        c = await sendCDP$1(n, "Runtime.callFunctionOn", {
          ...
          arguments: i.map((m) => ({ objectId: m })),
          returnByValue: !0,
          awaitPromise: !0
        });
      } catch (m) {
        const d = m instanceof Error ? m.message : String(m);
        throw /not.*function|does not evaluate/i.test(d) ? new Error("With args, code must be a function expression, e.g. (el) => el.innerText") : m;
      }
  ...
  const { pageSize: t, pageIdx: s, resourceTypes: o } = e, n = await sessionForCapture(e, "Network capture"), a = Array.isArray(o) ? o.map((d) => String(d).toLowerCase()) : [], r = a.length > 0 ? n.network.filter((d) => a.includes(d.type)) : n.network, { items: i, total: c, page: m } = paginate(r, t, s);
  return { requests: i.map(summarize), total: c, page: m };
  ...
  const { pageSize: t, pageIdx: s, types: o, includeStackTraces: n } = e, a = await sessionForCapture(e, "Console capture"), r = Array.isArray(o) ? o.map((u) => String(u).toLowerCase()) : [], i = r.length > 0 ? a.console.filter((u) => r.includes(u.type)) : a.console, { items: c, total: m, page: d } = paginate(i, t, s);
  ...
  await api$6.scripting.executeScript({
    target: { tabId: e },
    func: (a, r, i, c) => {
      const m = c === "right" ? 2 : c === "middle" ? 1 : 0, d = document.elementsFromPoint(a, r).find((u) => !u.closest("wolffish-overlay")) ?? document.body, l = (u) => {
        d.dispatchEvent(
          new MouseEvent(u, {
            bubbles: !0,
            cancelable: !0,
  ...
  if (a) {
    assertNoDialog(a);
    const { x: m, y: d } = await resolveTargetPoint(n, t, { scroll: !0 });
    return await sleep$1(gaussianDelay(50, 150)), await cdpMove(a, m, d), await cdpClick(a, m, d, s, o), { success: !0, x: m, y: d, trusted: !0 };
  }
  ...
  return o ? (assertNoDialog(o), await cdpMove(o, a.x, a.y), await cdpPress(o, a.x, a.y, "left", 1), await sleep$1(gaussianDelay(60, 140)), await cdpMove(o, r.x, r.y, !0), await sleep$1(gaussianDelay(60, 140)), await cdpRelease(o, r.x, r.y, "left", 1), { success: !0, x: r.x, y: r.y, trusted: !0 }) : (await api$6.scripting.executeScript({
    target: { tabId: s },
    func: (i, c, m, d) => {
      const l = (S, A) => document.elementsFromPoint(S, A).find((R) => !R.closest("wolffish-overlay")) ?? document.body, u = l(i, c), g = l(m, d), f = (S, A, R, I) => {
        I.dispatchEvent(
          new MouseEvent(S, { bubbles: !0, cancelable: !0, clientX: A, clientY: R, button: 0, view: window })
        );
      };
      f("mousedown", i, c, u), f("mousemove", Math.round((i + m) / 2), Math.round((c + d) / 2), g), f("mousemove", m, d, g), f("mouseup", m, d, g);
    },
  ...
  try {
    ...
    const m = await api$5.tabs.get(e).catch(() => null);
    return !!m && (m.status === "loading" || !!t && !!m.url && m.url !== t);
  } finally {
  ...
  let n = !1, a = null;
  const r = () => {
    c.disconnect(), a && clearTimeout(a), clearTimeout(m), s(n);
  }, i = () => {
...
}, api$4 = globalThis.chrome, WOLFFISH_GROUP_TITLE = "Wolffish", WOLFFISH_GROUP_COLOR = "blue", KEY_GROUP_ID = "wf:group-id", KEY_TAB_ID = "wf:tab-id", KEY_LABEL = "wf:group-label", stateArea$1 = () => api$4.storage.session ?? api$4.storage.local, readState = async (e) => {
  try {
    const t = await stateArea$1().get(e);
  ...
  try {
    await stateArea$1().set({ [e]: t });
  } catch {
  }
}, clearState = async (e) => {
  try {
    await stateArea$1().remove(e);
  } catch {
  }
}, formatLabel = (e) => {
  ...
  if (!api$4.tabGroups) return null;
  const e = await readState(KEY_GROUP_ID);
  return typeof e != "number" ? null : await api$4.tabGroups.get(e).then(() => !0).catch(() => !1) ? e : (await clearState(KEY_GROUP_ID), null);
}, paintGroup = async (e) => {
  ...
  if (!api$4.tabGroups || !api$4.tabs.group) return null;
  const t = await liveGroupId();
  if (t !== null)
    try {
      return await api$4.tabs.group({ groupId: t, tabIds: [e] }), await paintGroup(t), t;
    } catch {
      await clearState(KEY_GROUP_ID);
    }
  try {
    const s = await api$4.tabs.get(e), o = await api$4.tabs.group({ tabIds: [e], createProperties: { windowId: s.windowId } });
    return await writeState(KEY_GROUP_ID, o), await paintGroup(o), o;
  } catch (s) {
  ...
  return typeof e != "number" ? null : await api$4.tabs.get(e).then(() => !0).catch(() => !1) ? e : (await clearState(KEY_TAB_ID), null);
}, adoptTab = async (e) => {
  await writeState(KEY_TAB_ID, e), await groupTab(e);
}, openWorkspaceTab = async (e, t = !0) => {
  const o = (await api$4.tabs.create({ url: e || "about:blank", active: t })).id;
  return await adoptTab(o), o;
}, ensureWorkspaceTab = async () => {
  const e = await getWorkspaceTabId();
  return e !== null ? e : openWorkspaceTab();
}, rememberWorkspaceTab = async (e) => {
  const t = await api$4.tabs.get(e).catch(() => null);
  return !t || !await isWorkspaceTab(t) ? !1 : (await writeState(KEY_TAB_ID, e), !0);
}, getActivityLabel = async () => readState(KEY_LABEL), setActivity = async (e) => {
  var o, n;
  const t = { emoji: ((o = e.emoji) == null ? void 0 : o.trim()) || void 0, text: ((n = e.text) == null ? void 0 : n.trim()) || void 0 };
...
const setOverlayEnabled = async (e) => {
  ...
  const e = await getActivityLabel(), t = ((e == null ? void 0 : e.emoji) ?? "").trim(), s = ((e == null ? void 0 : e.text) ?? "").trim();
  if (!(!t && !s))
    return [t, s].filter(Boolean).join(" ");
}, markTabInUse = async (e, t) => {
  const s = inUse.get(e);
  inUse.set(e, { at: Date.now(), kind: t }), persist(), api$3.alarms.create(IDLE_ALARM, { delayInMinutes: OVERLAY_IDLE_ALARM_MINUTES }), enabled && (s && s.kind === t || await post(e, { type: "overlay", op: "pill", mode: pillMode(t), text: await pillText() }));
}, clearTab = async (e) => {
...
var B, k;
(k = (B = api$3.webNavigation) == null ? void 0 : B.onCompleted) == null || k.addListener(async (e) => {
  if (e.frameId !== 0 || !enabled) return;
  const t = inUse.get(e.tabId);
  if (!t || Date.now() - t.at > OVERLAY_IDLE_MS) return;
  await post(e.tabId, { type: "overlay", op: "pill", mode: pillMode(t.kind), text: await pillText() });
...
const overlayDriver = {
  ...
  execute: async (e) => {
    ...
    for (let a = 0; a < n; a++) {
      const r = gaussianDelay(-5, 5), i = gaussianDelay(-5, 5), c = Math.max(0, o.x + r), m = Math.max(0, o.y + i);
      await sendCDP(e, "Input.dispatchMouseEvent", {
        type: "mouseMoved",
        x: c,
        y: m
  ...
  var a, r, i, c, m;
  try {
    const d = globalThis.browser, l = await ((r = (a = d == null ? void 0 : d.runtime) == null ? void 0 : a.getBrowserInfo) == null ? void 0 : r.call(a));
    if (l != null && l.name)
      return { browser: l.name.toLowerCase(), browserName: l.name, browserVersion: l.version };
  } catch {
  }
  const s = ((i = e.userAgentData) == null ? void 0 : i.brands) ?? [], o = (d) => s.find((l) => l.brand.toLowerCase().includes(d)), n = [
  ...
  for (const { needle: d, slug: l, name: u } of n) {
    const g = o(d);
    if (g) return { browser: l, browserName: u, browserVersion: g.version || uaVersion(t, "Chrome") };
  }
  try {
    if (await ((m = (c = e.brave) == null ? void 0 : c.isBrave) == null ? void 0 : m.call(c)))
...
const connectWebSocket = async (e) => {
  ...
  const r = () => {
    var f, S;
    clearTimeout(l), clearInterval(d), (S = (f = api.webNavigation) == null ? void 0 : f.onCompleted) == null || S.removeListener(m);
  }, i = (f) => {
    n || (n = !0, r(), o(f));
  }, c = async () => {
  ...
  (g = (u = api.webNavigation) == null ? void 0 : u.onCompleted) == null || g.addListener(m);
  const d = setInterval(() => void c(), 100), l = setTimeout(() => {
    api.tabs.get(e).then(i).catch(() => i(null));
  }, s);
}), handleNavigate = async (e) => {
  const { url: t, waitUntil: s, newTab: o } = e, n = o ? await openWorkspaceTab() : await resolveTabId(e), a = await api.tabs.get(n).catch(() => null), r = (a == null ? void 0 : a.url) ?? "";
  ...
  const { windowId: t } = e, s = t !== void 0 ? { windowId: t } : {}, o = await api.tabs.query(s), n = await getWorkspaceGroupId();
  return {
    tabs: o.map((a) => ({
      id: a.id,
      url: a.url || "",
      ...
      windowId: a.windowId,
      groupId: a.groupId,
      wolffish: n !== null && a.groupId === n
    }))
  };
}, handleTabOpen = async (e) => {
  const { url: t, active: s } = e, o = await openWorkspaceTab(t, s ?? !0), n = await api.tabs.get(o).catch(() => null);
  return {
    tabId: o,
  ...
  let m = await api.tabs.get(r);
  m.active || (await api.tabs.update(r, { active: !0 }), await new Promise((S) => setTimeout(S, 150)), m = await api.tabs.get(r)), await overlayHooks.beforeCapture(r).catch(() => {
  });
  let d;
  try {
    d = await api.tabs.captureVisibleTab(m.windowId, c);
  } finally {
  ...
  const l = await api.scripting.executeScript({
    ...
    var A;
    return (A = S[0]) == null ? void 0 : A.result;
  }).catch(() => {
  ...
  const { url: t, name: s, value: o, domain: n, path: a, expires: r, httpOnly: i, secure: c } = e, m = { url: t, name: s, value: o };
  return n !== void 0 && (m.domain = n), a !== void 0 && (m.path = a), r !== void 0 && (m.expirationDate = r), i !== void 0 && (m.httpOnly = i), c !== void 0 && (m.secure = c), await api.cookies.set(m), { success: !0 };
  ...
  return r === 0 ? { downloadId: a, state: "in_progress" } : new Promise((i) => {
    ...
    const m = (g) => {
      ...
      g && (g.state === "complete" ? m({ downloadId: a, state: "complete", filename: g.filename }) : g.state === "interrupted" && m({ downloadId: a, state: "interrupted", error: g.error ?? "interrupted" }));
    }, l = (g) => {
      g.id === a && d();
    };
    api.downloads.onChanged.addListener(l);
    const u = setTimeout(() => m({ downloadId: a, state: "in_progress" }), r);
    d();
  });
  ...
  return new Promise((a) => {
    let r = !1;
    const i = (l, u) => {
      r || (r = !0, clearTimeout(c), api.webNavigation.onCompleted.removeListener(m), api.tabs.onUpdated.removeListener(d), a({ url: l, title: u }));
    }, c = setTimeout(() => {
    ...
    api.webNavigation.onCompleted.addListener(m), api.tabs.onUpdated.addListener(d);
  });
}, handleWait = async (e) => {
  ...
  try {
    ...
    try {
      await api.scripting.executeScript({ target: { tabId: l }, func: () => !0 }), m = { tabId: l, ok: !0, url: u == null ? void 0 : u.url };
    } catch (g) {
      const f = g instanceof Error ? g.message : String(g);
      d = /policy|ExtensionSettings|blocked by/i.test(f), m = { tabId: l, ok: !1, error: f, url: u == null ? void 0 : u.url };
    }
  } catch {
  }
  return {
  ...
  try {
    let o;
    const n = commandKind(e.type);
    let a = null;
    if (TAB_AGNOSTIC.has(e.type) || (a = await resolveTabId(e.params).catch(() => null)), a !== null && DIALOG_BLOCKED_COMMANDS.has(e.type)) {
      const i = dialogOpenError(a);
      if (i) {
        sendResponseToServer(makeErrorResponse(e.id, i)), log("→", e.type, "blocked by dialog");
        return;
      }
    }
    a !== null && n && markTabInUse(a, n);
    const r = a !== null && INPUT_COMMANDS.has(e.type) ? await captureBefore(a) : void 0;
    if (SERVICE_WORKER_COMMANDS.has(e.type)) {
      const i = SERVICE_WORKER_HANDLERS[e.type];
      if (!i)
        o = makeErrorResponse(e.id, `No handler for command: ${e.type}`);
      else {
        const c = await withTimeout(i(e.params));
        o = makeResponse(e.id, await decorate(e.type, a, c, r));
      }
    } else if (CONTENT_SCRIPT_COMMANDS.has(e.type)) {
      if (a !== null && hasSession(a) && DEBUGGER_ROUTABLE_COMMANDS.has(e.type)) {
        const m = CDP_HANDLERS[e.type];
        if (m)
          try {
            const d = await withTimeout(m({ ...e.params, tabId: a }));
            o = makeResponse(e.id, await decorate(e.type, a, d, r)), log("→", e.type, "success (CDP)"), sendResponseToServer(o);
            return;
          } catch (d) {
            const l = d instanceof Error ? d.message : String(d);
            if (typeof ((t = e.params) == null ? void 0 : t.uid) == "string" || typeof ((s = e.params) == null ? void 0 : s.from_uid) == "string") {
              sendResponseToServer(makeErrorResponse(e.id, l)), log("→", e.type, "CDP error (uid target, no fallback):", l);
              return;
            }
            log("CDP fallback:", e.type, l);
          }
      }
      const i = a ?? await resolveTabId(e.params);
      await ensureContentScriptInjected(i);
      const c = await withTimeout(
        sendToContentScript(i, {
          source: "service-worker",
          target: "content-script",
          payload: e
        })
      );
      o = (c == null ? void 0 : c.success) === !0 ? makeResponse(e.id, await decorate(e.type, i, c.data, r)) : c;
    } else
      o = makeErrorResponse(e.id, `Unknown command: ${e.type}`);
    log("→", e.type, o.success ? "success" : o.error), sendResponseToServer(o);
  } catch (o) {
    const n = o instanceof Error ? o.message : String(o), a = makeErrorResponse(e.id, n);
