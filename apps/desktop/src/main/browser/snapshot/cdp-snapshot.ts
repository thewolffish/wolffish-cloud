/**
 * Accessibility snapshot for the in-app browser.
 *
 * Ported from wolffish-extension chrome-extension/src/background/
 * cdp-snapshot.ts (extension 0.1.66). Same three CDP reads
 * (Accessibility.getFullAXTree + DOM.getDocument{pierce} + a per-iframe
 * graft), same merge on backendNodeId, same line grammar, same uid contract:
 *
 *   uid=<snapshotId>_<n>   a label over `loaderId:backendNodeId`, kept across
 *                          re-snapshots of the same document, evicted on
 *                          navigation
 *
 * so the model drives this browser with exactly the mental model it already
 * has for `ext_*`. Two deliberate divergences from the source, both at the
 * seam rather than in the algorithm: the Chrome `tabId` became an injected
 * `CdpSend`, and the stale-uid error phrases name `preview_snapshot` instead
 * of `ext_take_snapshot` — this copy's results never pass through the
 * extension plugin's classifier, they pass through the preview capability's,
 * which matches the same "Take a new snapshot" / "first" shapes.
 *
 * Cross-origin iframes are a wall line here exactly as they are in the
 * extension. Threading Target.setAutoAttach session ids through CdpSend is
 * the additive follow-up that would open them.
 */
import { center, nodeRect } from '@main/browser/snapshot/cdp-dom'
import type {
  CdpSend,
  FoundElement,
  SnapshotNode,
...
/** Kept so an on-page cursor overlay, if one is ever drawn, is hidden from the model. */
...
const fetchAXNodes = async (send: CdpSend, frameId?: string): Promise<AXNode[]> => {
  const res = (await send('Accessibility.getFullAXTree', frameId ? { frameId } : {})) as {
    nodes?: AXNode[]
  }
  return res.nodes ?? []
}

/** Roles lowercased — except the document root, spelled `RootWebArea` everywhere. */
...
/** One `inlinetextbox` per rendered line of a text node — noise. */
const NOISE_ROLES = new Set(['inlinetextbox', 'linebreak'])

/**
 * Same-process child frames sometimes arrive inline and sometimes not. When
 * an iframe node has no children but the DOM shows a content document, fetch
 * that frame's tree and graft it under the iframe node, namespacing ids.
 */
const graftChildFrames = async (
  send: CdpSend,
  byId: Map<string, AXNode>,
  dom: Map<number, DomInfo>
): Promise<void> => {
  const iframes = [...byId.values()].filter(
    (n) => roleOf(n) === 'iframe' && !(n.childIds && n.childIds.length > 0)
  )
  for (const iframe of iframes) {
    const info =
      iframe.backendDOMNodeId !== undefined ? dom.get(iframe.backendDOMNodeId) : undefined
    const frameId = info?.frameId ?? iframe.frameId
    if (!info?.hasContentDocument || !frameId) continue
    let nodes: AXNode[]
    try {
      nodes = await fetchAXNodes(send, frameId)
    } catch {
      continue
    }
...
const attributesFor = (node: AXNode, role: string, info: DomInfo | undefined): string => {
  ...
  const value =
    axValue !== undefined && axValue !== '' ? String(axValue) : (info?.attrs.value ?? '')
  ...
  if (role === 'link' && info?.attrs.href)
    parts.push(`href="${escapeText(clip(info.attrs.href, 200))}"`)
...
const assignUid = (ctx: BuildContext, node: AXNode): { uid: string; isNew: boolean } => {
  const key =
    node.backendDOMNodeId !== undefined
      ? `${ctx.session.loaderId}:${node.backendDOMNodeId}`
      : `ax:${node.nodeId}`
...
const walk = (ctx: BuildContext, node: AXNode, depth: number, ancestorNames: string[]): void => {
  ...
  if (
    ...
    !ctx.verbose &&
    role === 'statictext' &&
    ancestorNames.includes(name.trim().toLowerCase())
  )
    keep = false
...
export const takeSnapshot = async (
  ...
  const [axNodes, doc] = await Promise.all([
    fetchAXNodes(session.send),
    session.send('DOM.getDocument', { depth: -1, pierce: true }) as Promise<{ root: DomNode }>
  ])
  const { dom, overlay } = indexDom(doc.root)
  const byId = new Map<string, AXNode>(axNodes.map((n) => [n.nodeId, n]))
  await graftChildFrames(session.send, byId, dom)

  const snapshotId = ++session.snapshotSeq
...
export const NO_SNAPSHOT_ERROR = 'No snapshot for this page. Call preview_snapshot first.'

export const uidNotFoundError = (uid: string): string =>
  `Element uid "${uid}" not found in the latest snapshot. Take a new snapshot with preview_snapshot.`

export const uidDetachedError = (uid: string): string =>
  `Element uid "${uid}" was detached or no longer exists on the page. Take a new snapshot with preview_snapshot.`

export const lookupUid = (session: SnapshotSession, uid: string): UidRef => {
...
export const withUidErrors = async <T>(uid: string, op: () => Promise<T>): Promise<T> => {
  try {
    return await op()
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    if (
      /no node|not found|could not find|detached|does not have a layout object|invalid/i.test(
        message
      )
    ) {
...
export const findInSnapshot = async (
  session: SnapshotSession,
  query: string,
  limit: number
  ...
  for (const { node, score } of ranked) {
    try {
      const rect = await nodeRect(session.send, node.backendNodeId)
      out.push({
        uid: node.uid,
        tag: node.tag,
        role: node.role,
