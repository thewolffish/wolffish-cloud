 ...
 *    `wfc` that answered every command with a blank line. The shim
 *    delegates to `wfc-cli.exe`, which is console-subsystem and passes its
 *    own handles down explicitly. See build/win-cli-launcher/wfc-cli.cs.
 *  - Booting ~150 MB of Electron to run `wfc ls` is absurd. The shim
...
/**
 * The pre-`~/.wfc/bin` location. Swept on install so an upgraded machine
...
export function cliEntryPath(isDev: boolean, appPath: string, resourcesPath: string): string {
  return isDev
    ? path.join(appPath, 'src', 'cli', 'wfc.mjs')
    : path.join(resourcesPath, 'cli', 'wfc.mjs')
}
...
function launcherPath(execPath: string): string {
  return path.join(path.dirname(execPath), 'wfc-cli.exe')
}
 ...
 * for `wfc` never considers `wfc.cmd` and reports "command not
 ...
 * `wfc.cmd` through PATHEXT and never see this one (an extensionless file
...
`
  ...
  // redirect or a pipe — `wfc -p ... | grep` and `cat log | wfc` must
...
function windowsShim(execPath: string, entry: string): string {
  ...
  // app binary from a .cmd directly is what made `wfc` print NOTHING: it
  // is GUI-subsystem, so an unredirected cmd.exe gives it no usable stdout and
  // the output goes nowhere. See build/win-cli-launcher/wfc-cli.cs.
  const launcher = launcherPath(execPath)
  if (existsSync(launcher)) {
