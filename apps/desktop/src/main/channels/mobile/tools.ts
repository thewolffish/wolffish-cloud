 ...
 * the mobile channel manages directly.
 */
...
export function buildMobileCapability(deps: ToolDeps): {
  ...
  const tools: SkillToolDescriptor[] = [
    {
      name: 'notify_phone',
      // A push complements the reply and changes nothing the user is
      // working on, so a read-only turn (plan mode) may still close with one.
      readOnly: true,
      description: `Send a push notification to the user's paired phone — their lock screen, their pocket. It lands even when the mobile app is closed or backgrounded, and when the phone is offline it arrives as soon as signal returns (iOS and Android alike), so this reaches the user when nothing else does. This tool being available already means a phone is paired and notifications are allowed. NOTIFYING IS THE DEFAULT — silence is what needs a reason. END EVERY TURN WITH ONE CALL, as the LAST tool of your wrap-up: after the work is done and any files are delivered — phase 'completed' (or 'failed'), deeplink "${DEEPLINK_SCHEME}chat?id=${CURRENT_CONVERSATION}". A turn the user watched you finish still gets one; they will be somewhere else five minutes from now, and the phone is a second surface, not a mirror of the screen they happen to be at. THIS TOOL NEVER EARNS A SECOND REPLY: your wrap-up is written BEFORE it, and once it lands the turn is over — do not restate, summarise or reword what you just wrote. If you find yourself composing prose after it, you have already said that news: end that reply with zero characters. A second closing paragraph is the same news twice on the user's screen. Send one MID-turn too, without waiting for the end, the moment something major happens: you are blocked on their input or approval ('needs_input' — the moment you block; it expires in minutes), a step fails in a way that changes the plan, or you turn up a finding they would act on now rather than in ten minutes. Long work (a big build, scrape, render, video, multi-step automation) also earns a 'started' so they know it is underway, and a scheduled automation always closes its digest with one 'completed'. The BODY must carry real information — the outcome, the number, what is next: 'AI news is ready — 7 stories, 2 worth your time' or 'Sheet rebuilt — 214 rows, 3 broken dates fixed' earn the buzz; 'Task completed' and 'Done' waste it. Stay silent only when there is genuinely nothing to carry (small talk, a one-line acknowledgment, an answer the notification would repeat word for word), for mid-turn narration ('opened the file', 'tests running' — prose does that job), or when the user has asked for quiet. A notification COMPLEMENTS your conversation reply, never replaces any part of it: it is not shown in the conversation, so the reply must stand complete on its own and never lean on the notification. NEVER THE SAME NEWS TWICE — nothing here counts your notifications or refuses a repeat, so every call lands on the user's phone: one call per moment, never a reworded repeat, and never a retry loop. An answer reporting delivery as UNCONFIRMED means it may already be on their lock screen, so sending it again — reworded, re-phased, or verbatim — is how one notification becomes three in someone's pocket.`,
      parameters: {
        title: {
          type: 'string',
