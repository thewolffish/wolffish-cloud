...
import { authGateMessage } from '@main/channels/cli/auth-gate'
import type { AuthState } from '@main/cloud/session'
...
/** `~/.wfc/workspace/cli-state.json` — see the cli:kv* handlers. */
...
export type CliIpcDeps = {
  ...
  /**
   * The redacted account state (no tokens) — what `wfc account` prints and
   * what gates `cli:send`. Optional so the seam tests can register without a
   * cloud session; a daemon always supplies it.
   */
  auth?: () => AuthState
...
export function registerCliIpc(deps: CliIpcDeps): void {
   ...
   * the daemon owns `~/.wfc`, so the daemon owns this file too, and a
  ...
  handle(
    ...
    async (
      ...
      payload: {
        ...
        /** Plan mode: read-only turn that may only write its plan file (`wfc --plan`). */
        planMode?: boolean
      }
    ) => {
      // Signed out, locked, or half signed-in: refuse here with the fix, not
      // three iterations later with "authentication failed".
      const gate = deps.auth ? authGateMessage(deps.auth().status) : null
