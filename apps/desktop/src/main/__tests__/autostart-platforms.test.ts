...
async function main(): Promise<void> {
  ...
  check('linux desktop: writes ~/.config/autostart/wfc.desktop', () => {
  ...
  const unitFile = path.join(serverHome, '.config', 'systemd', 'user', 'wfc.service')
  ...
  const plistFile = path.join(macHome, 'Library', 'LaunchAgents', 'cloud.wolffi.sh.plist')
  ...
  check('macos: uninstall removes the plist and clears active', () => {
    ...
    assert.ok(
      !fs.existsSync(path.join(statusHome, 'Library', 'LaunchAgents', 'cloud.wolffi.sh.plist')),
  ...
  const reversible: Array<[NodeJS.Platform, string, string]> = [
    ['darwin', 'Library/LaunchAgents/cloud.wolffi.sh.plist', 'login item'],
    ['linux', '.config/systemd/user/wfc.service', '.desktop entry']
  ]
  ...
  const shimCases: Array<[NodeJS.Platform, string, string]> = [
    ['darwin', path.join(TMP, 'shim-mac'), '.wfc/bin/wfc'],
    ['linux', path.join(TMP, 'shim-linux'), '.wfc/bin/wfc'],
    ['win32', path.join(TMP, 'shim-win'), '.wfc/bin/wfc.cmd']
  ]
  for (const [platform, home, relative] of shimCases) {
    ...
    await asPlatform(platform, home, () =>
      cliPath.installCliPath(
        ...
        platform === 'win32'
          ? 'C:\\Users\\a b\\resources\\cli\\wfc-cli-win32-x64.exe'
          : '/res/cli/wfc-cli-darwin-arm64'
      )
    )
    ...
    check(`${platform}: shim runs the compiled client and names the app for autostart`, () => {
      ...
      assert.ok(body.includes('wfc-cli'), 'must reach the compiled client')
    })
    if (platform === 'win32') {
  ...
  const winBinary = path.join(launcherInstall, 'resources', 'cli', 'wfc-cli-win32-x64.exe')
  ...
  check('windows: shim runs wfc-cli-win32-x64.exe, never the GUI binary', () => {
    const body = fs.readFileSync(path.join(launcherHome, '.wfc', 'bin', 'wfc.cmd'), 'utf8')
    assert.ok(body.includes('wfc-cli-win32-x64.exe'), `never reached the client:\n${body}`)
    assert.ok(
  ...
  // PATHEXT is a cmd.exe convention. Bash looking for `wfc` never
  // considers `wfc.cmd`, so Git Bash users — a large share of Windows
  ...
  // looks for the .cmd.
  const bashShim = path.join(launcherHome, '.wfc', 'bin', 'wfc')
  check('windows: an extensionless shim is written for Git Bash', () => {
    ...
    assert.ok(fs.existsSync(path.join(launcherHome, '.wfc', 'bin', 'wfc.cmd')))
  })
  check('windows: the Git Bash shim is a sh script MSYS will execute', () => {
    ...
    assert.ok(body.includes('"$@"'), 'must forward arguments quoted')
    assert.ok(body.includes('wfc-cli-win32-x64.exe'), 'must reach the compiled client')
  })
  check('windows: the Git Bash shim uses forward slashes', () => {
    const body = fs.readFileSync(bashShim, 'utf8')
    const launcherLine = body.split('\n').find((line) => line.includes('wfc-cli-win32-x64.exe'))
    assert.ok(
  ...
  check('windows: winpty is used for a terminal, never for a pipe', () => {
    ...
    // redirect would corrupt `wfc -p ... | grep`, so it is gated on both
  ...
  // `wfc` working in Git Bash and Remove looks like a no-op.
  ...
  // called `wfc.exe`. So every Windows install resolved the name to a
  ...
  fs.writeFileSync(path.join(installRoot, 'wfc.exe'), Buffer.alloc(9000))
  const originalCwd = process.cwd()
  const cwdTrapStatus = await asPlatform('win32', cwdHome, async () => {
    await cliPath.installCliPath(path.join(installRoot, 'wfc.exe'), '/res/cli.mjs')
  ...
  check('windows: the shim wins the name even when the cwd holds wfc.exe', () => {
    assert.equal(
      cwdTrapStatus.resolved,
      path.join(cwdHome, '.wfc', 'bin', 'wfc.cmd'),
  ...
  for (const [platform, home] of [
    ...
    check(`${platform}: shim install writes nothing outside ~/.wfc but the PATH entry`, () => {
      ...
      for (const entry of stray) {
        ...
        assert.ok(body.includes('Wolffish Cloud CLI — added by the app'), `${entry} is not ours`)
      }
    })
  }
  ...
  check("path entry: the profile gets ONE marked block, after the user's own lines", () => {
    ...
    assert.equal(body.split('Wolffish Cloud CLI — added by the app').length, 2, body)
    assert.ok(body.includes('export PATH="$HOME/.wfc/bin:$PATH"'), body)
  })
  check('path entry: fish gets its conf.d file when fish is set up', () => {
    const fish = path.join(entryHome, '.config', 'fish', 'conf.d', 'wfc.fish')
  ...
  check('path entry: uninstall takes exactly the block back out', () => {
    ...
    assert.ok(!body.includes('Wolffish Cloud CLI'), body)
    assert.ok(!fs.existsSync(path.join(entryHome, '.config', 'fish', 'conf.d', 'wfc.fish')))
  ...
  // An upgraded machine must not keep the shims this app wrote under its
  // FORMER command name (`wolffish`, before the rename to `wfc`): they sit in
  // the same ~/.wfc/bin and would keep answering the old name with a stale
  // target. They are this app's files — the directory is exclusively Wolffish
  // Cloud's — so no signature check is needed.
  const upgradeHome = path.join(TMP, 'upgrade')
  const upgradeBin = path.join(upgradeHome, '.wfc', 'bin')
  fs.mkdirSync(upgradeBin, { recursive: true })
  fs.writeFileSync(path.join(upgradeBin, 'wolffish'), '#!/bin/sh\nexec /old/path "$@"\n')
  fs.writeFileSync(path.join(upgradeBin, 'wolffish.cmd'), '@echo off\r\n"/old/path" %*\r\n')
  await asPlatform('darwin', upgradeHome, () =>
    cliPath.installCliPath('/Applications/Wolffish.app/Contents/MacOS/Wolffish', '/res/cli.mjs')
  )
  check('upgrade: the old-name shims in ~/.wfc/bin are swept', () => {
    assert.ok(!fs.existsSync(path.join(upgradeBin, 'wolffish')), 'stale `wolffish` shim survived')
    assert.ok(
      !fs.existsSync(path.join(upgradeBin, 'wolffish.cmd')),
      'stale `wolffish.cmd` survived'
    )
    assert.ok(fs.existsSync(path.join(upgradeBin, 'wfc')))
  })

  // ...and nothing outside ~/.wfc is touched: a `wolffish` in ~/.local/bin is
  // the personal Wolffish app's (or the user's own) and is not this app's to
  ...
  fs.writeFileSync(
    path.join(foreignDir, 'wolffish'),
    '#!/bin/sh\n# Wolffish CLI launcher — generated by the app\nexec /old/path "$@"\n'
  )
  await asPlatform('darwin', foreignHome, () =>
    cliPath.installCliPath('/Applications/Wolffish.app/Contents/MacOS/Wolffish', '/res/cli.mjs')
  )
  check("upgrade: the personal app's ~/.local/bin shim is left alone", () => {
    assert.ok(
      fs.existsSync(path.join(foreignDir, 'wolffish')),
      "deleted a file outside ~/.wfc — the other app's command just broke"
    )
  })
  ...
  fs.writeFileSync(path.join(mountCli, 'wfc-cli-linux-x64'), 'ELF', { mode: 0o755 })
  ...
  await asPlatform('linux', appImageHome, () =>
    cliPath.installCliPath(path.join(mount, 'wfc-app'), path.join(mountCli, 'wfc-cli-linux-x64'))
  )
  const appImageShim = fs.readFileSync(path.join(appImageHome, '.wfc', 'bin', 'wfc'), 'utf8')
  ...
  check('appimage: the client is lifted out of the mount, imports included', () => {
    const copied = path.join(appImageHome, '.wfc', 'cli', 'wfc-cli-linux-x64')
  ...
  check('appimage: the copy respects the rm -rf ~/.wfc rule', () => {
    ...
    const stray = fs
      ...
      .filter((entry) => entry !== '.wfc' && !profiles.has(entry))
  ...
  // launcher it writes. The shim has to reach the same conclusion or `wfc`
  ...
  check('appimage: a self-extracting launch is carried into the shim', () => {
    const body = fs.readFileSync(path.join(extractHome, '.wfc', 'bin', 'wfc'), 'utf8')
    assert.ok(
  ...
  await asPlatform('linux', extractSvc, () =>
    autostart.installAutostart('headless', path.join(mount, 'wfc-app'))
  )
  ...
  delete process.env.XDG_CONFIG_HOME
  await asPlatform('linux', extractSvc, () =>
    autostart.installAutostart('gui', path.join(mount, 'wfc-app'))
  )
  ...
  check('appimage: the systemd unit carries the self-extract environment', () => {
    const body = fs.readFileSync(
      path.join(extractSvc, '.config', 'systemd', 'user', 'wfc.service'),
      'utf8'
    )
    assert.ok(
  ...
  check('appimage: the XDG entry carries it as env, not as a bare assignment', () => {
    const body = fs.readFileSync(
      path.join(extractSvc, '.config', 'autostart', 'wfc.desktop'),
      'utf8'
    )
    const exec = body.split('\n').find((line) => line.startsWith('Exec='))
    assert.ok(
  ...
  check('appimage: the systemd unit starts the .AppImage, not the mount', () => {
    const body = fs.readFileSync(
      path.join(appImageUnitHome, '.config', 'systemd', 'user', 'wfc.service'),
      'utf8'
    )
    assert.ok(
  ...
  await asPlatform('linux', nativeHome, () =>
    cliPath.installCliPath('/opt/Wolffish/wfc-app', '/opt/Wolffish/resources/cli/wfc-cli-linux-x64')
  )
  check('native install: no client copy, shim points straight into /opt', () => {
    ...
    const body = fs.readFileSync(path.join(nativeHome, '.wfc', 'bin', 'wfc'), 'utf8')
  ...
  // typing at it. wfc-cli.exe passes the truth in as environment; these
  ...
  // `type log.txt | wfc -p "why?"` — the documented usage — dropped the
  ...
  check('dev: the shim runs src/cli/index.ts under the repo bun with the Solid preload', () => {
    const body = fs.readFileSync(path.join(devHome, '.wfc', 'bin', 'wfc'), 'utf8')
  ...
  check('dev windows: both shims run the source under bun.exe', () => {
    const cmd = fs.readFileSync(path.join(devWinHome, '.wfc', 'bin', 'wfc.cmd'), 'utf8')
    assert.ok(cmd.includes('bun.exe') && cmd.includes('--preload') && cmd.includes('index.ts'), cmd)
    const bash = fs.readFileSync(path.join(devWinHome, '.wfc', 'bin', 'wfc'), 'utf8')
