...
├── cli/             the `wfc` terminal client (Bun + OpenTUI + Solid) — talks to main over the CLI socket
...
│   ├── commands/, lib/  classic line-mode verbs (.mjs) reused by the client
│   └── build.ts     `bun build --compile` (`npm run cli:build` → build/cli/host; client forks wire it into their packaging)
│                    DEV: `npm run dev` writes a `wfc` shim that runs src/cli/index.ts under Bun —
│                    every launch is the source on disk, no build; relaunch + `wfc resume` after edits
├── renderer/src/
