 ...
 * one password for all seeded people is taken from WFC_DEMO_PASSWORD or
...
const REASONING_MODEL = 'deepseek-ai/DeepSeek-R1-0528'
...
// One password for the whole demo cast — never committed anywhere.
const password =
  process.env.WFC_DEMO_PASSWORD ?? `demo-${randomBytes(9).toString('base64url')}`
...
lines.push(
  `INSERT INTO org (id, name, default_model, default_allowed_models, user_daily_token_cap, org_monthly_token_cap)
   VALUES (1, 'Wolffish Inc', '${DEFAULT_MODEL}', '${JSON.stringify([DEFAULT_MODEL, REASONING_MODEL])}', 2000000, 500000000)
...
for (const person of roster) {
  ...
  lines.push(
    `INSERT INTO users (id, email, name, role, status, password_hash, password_salt, must_change_password, temp_password_expires_at)
     VALUES ('${uid(person.n)}', '${esc(person.email)}', '${esc(person.name)}', '${person.role}', '${status}', '${hash}', '${salt}', ${mustChange},
       ${person.n === 49 ? "strftime('%Y-%m-%dT%H:%M:%fZ','now','+365 days')" : 'NULL'})
     ON CONFLICT(email) DO UPDATE SET password_hash = excluded.password_hash,
       password_salt = excluded.password_salt, role = excluded.role, status = excluded.status;`
  )
}
...
for (const person of roster) {
  ...
  for (let r = 0; r < requests; r++) {
    ...
    const model = rnd() < 0.8 ? DEFAULT_MODEL : REASONING_MODEL
    const tin = 200 + Math.floor(rnd() * 4000)
    const tout = 100 + Math.floor(rnd() * 2500)
    const cost = Math.round((tin * 0.27 + tout * 1.0))
    ...
    lines.push(
      `INSERT INTO usage (user_id, device_id, model, tokens_in, tokens_out, cost_microusd, latency_ms, decision, created_at)
       VALUES ('${uid(person.n)}', 'dev_demo_${person.n}', '${model}', ${denied ? 0 : tin}, ${denied ? 0 : tout},
         ...
         strftime('%Y-%m-%dT%H:%M:%fZ','now','-${daysAgo} days','+${hour} hours','-${Math.floor(rnd() * 3600)} seconds'));`
    )
  }
}
...
console.log('   ^ printed once, never stored. Rotate any account via admin reset-password.')
