...
export function CountdownCard({ snapshot }: { snapshot: CountdownSnapshot }): React.JSX.Element {
  ...
  // The transport is pulled in at press time, not at import time: bridgeClient
  ...
  const abort = async (): Promise<void> => {
    ...
    setAborting(true)
    try {
      const { bridgeClient } = await import('@/lib/cloud/bridge')
      const client = bridgeClient.active
      if (client && bridgeClient.connected) {
