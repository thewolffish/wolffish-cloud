...
describe('Channels — the terminal', () => {
  ...
  it('reports the desktop machine rather than offering to change it', async () => {
    await draw(<ChannelsScreen />)
    expect(screen.getByText('The wfc command')).toBeTruthy()
  ...
  it('switching the terminal feed on writes cliVerbose to the desktop', async () => {
    await draw(<ChannelsScreen />)
    // The second "Show all tool activity" on the screen: in-app, then the
    // terminal's, in the order the cards render.
    pressSegment('Show all tool activity', 'On', 1)
    await waitFor(() => {
  ...
  it('a change made on the desktop moves the row without a remount', async () => {
    await draw(<ChannelsScreen />)
    const row = (): unknown => screen.getAllByLabelText('Show all tool activity')[1]
    expect(
