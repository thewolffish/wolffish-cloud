import { REASONING_MODES, type ReasoningMode } from '@/lib/bridge/protocol'
...
export function useChatReasoning(): {
  ...
  return useMemo(() => {
    const source = Array.isArray(rawModes) && rawModes.length > 0 ? rawModes : REASONING_MODES
    const modes = source.filter((mode): mode is ReasoningMode =>
      (REASONING_MODES as readonly string[]).includes(mode)
    )
    const wanted = thinkingMode as ReasoningMode
    const current = modes.includes(wanted)
      ? wanted
      : // Prefer the model's default "on" rung when the stored pick is not one
        // this model offers; never silently up-shift into max.
        modes.includes('high')
        ? 'high'
        : (modes[0] ?? 'off')
